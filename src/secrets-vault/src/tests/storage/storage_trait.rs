...
async fn test_store_empty_data() -> anyhow::Result<()> {
    let crypto = DefaultCryptoFactory {}.new_crypto()?;
    let result =
        create_secret(b"", SecretDataType::Ed25519PvtKey, "empty_key", Algorithm::Ed25519, crypto)
            .await;
    assert!(result.is_err());
...
async fn test_compare_key_size() -> anyhow::Result<()> {
    ...
    for config in fixture() {
        ...
        assert!(keypair.eq_keypair(base_keypair)?);
...
async fn test_blob_multiple_operations_sequence() -> anyhow::Result<()> {
    ...
    for config in fixture() {
        ...
        assert!(blob2_loaded.eq_secret(&blob2_updated)?);

        let blob2_loaded_data = blob2_loaded.as_blob()?.data();
...
async fn test_mixed_blob_and_keypair_storage() -> anyhow::Result<()> {
    ...
    for config in fixture() {
        ...
        assert!(blob_loaded.eq_secret(&blob)?);
        ...
        assert!(keypair_loaded.eq_secret(&keypair)?);
...
async fn test_not_extractable_keypair_private_key_denied() -> anyhow::Result<()> {
    for config in fixture() {
        ...
        assert!(!loaded_keypair.extractable());
        assert!(loaded_keypair.private_key().is_err());
    }
...
#[tokio::test]
#[serial_test::serial]
async fn test_base64_special_symbol() -> anyhow::Result<()> {
    let crypto = DefaultCryptoFactory {}.new_crypto()?;
    ...
    for config in fixture() {
        ...
        let secret = create_secret(
            make_ed25519_test_key_32().as_ref(),
            SecretDataType::Ed25519PvtKey,
            "GEpySlMQ/GeNYV+fY27s0Z3gxniz1eAKBlWrbPcl3b4=",
            Algorithm::Ed25519,
            crypto.clone(),
        )
        .await?;
