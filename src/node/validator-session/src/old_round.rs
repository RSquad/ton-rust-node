...
use crate::{
    ...
    HashType, HashableObject, Merge, MovablePoolObject, OldRoundState, OldRoundStatePtr,
    OldRoundStateWrapper, PoolObject, PoolPtr, RoundState, RoundStatePtr, SentBlockPtr,
    SentBlockWrapper, SessionCache, SessionDescription, SessionFactory, SessionPool, VectorMerge,
    SKIP_ROUND_CANDIDATE_BLOCKID,
};
...
impl OldRoundState for OldRoundStateImpl {
    ...
    */
    ...
    fn merge_round(
        ...
        assert!(
            left.accelerated_consensus_collator_idx
                == right.get_accelerated_consensus_collator_index()
        );
        ...
        let approve_signatures = match signed_block {
            Some(signed_block) => {
                left.approve_signatures.merge(signed_block.get_approvers_list(), desc)
            }
...
impl Merge<PoolPtr<dyn OldRoundState>> for PoolPtr<dyn OldRoundState> {
    fn merge(&self, right: &Self, desc: &mut dyn SessionDescription) -> Self {
        ...
        let approve_signatures = left.approve_signatures.merge(&right.approve_signatures, desc);
...
fn get_impl(value: &dyn OldRoundState) -> &OldRoundStateImpl {
    value.get_impl().downcast_ref::<OldRoundStateImpl>().unwrap()
}
    ...
    */
    ...
    fn apply_approved_block_action(
        ...
        let approve_signatures =
            self.approve_signatures.change(desc, src_idx as usize, approve_signature);
    ...
    fn apply_committed_block_action(
        ...
        let commit_signature =
            SessionFactory::create_block_candidate_signature(desc, message.signature.clone());
        let commit_signatures = self.signatures.change(desc, src_idx as usize, commit_signature);
