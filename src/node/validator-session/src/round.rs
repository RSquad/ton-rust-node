...
//TODO: remove Option; return always ton::Message for create_action

use crate::{
    ton, Any, BlockCandidate, BlockCandidatePtr, BlockCandidateSignaturePtr,
    BlockCandidateSignatureVectorPtr, BlockCandidateWrapper, BlockId, CacheObject,
    CachedInstanceCounter, HashType, HashableObject, Merge, MovablePoolObject, PoolObject, PoolPtr,
    RoundAttemptState, RoundAttemptStatePtr, RoundAttemptStateWrapper, RoundState, RoundStatePtr,
    RoundStateWrapper, SentBlockPtr, SentBlockWrapper, SessionCache, SessionDescription,
    SessionFactory, SessionPool, SortedVector, SortedVectorWrapper, SortingPredicate,
    ValidatorWeight, Vector, VectorMerge, VectorWrapper, SKIP_ROUND_CANDIDATE_BLOCKID,
...
pub(crate) struct BlockCandidateSortingPredicate;
...
pub(crate) struct AttemptSortingPredicate;
...
pub(crate) type AttemptIdVector = Option<PoolPtr<dyn Vector<u32>>>;
pub(crate) type ApproveVector =
    Option<PoolPtr<dyn SortedVector<PoolPtr<dyn BlockCandidate>, BlockCandidateSortingPredicate>>>;
pub(crate) type AttemptVector =
...
pub(crate) struct RoundStateImpl {
    ...
    accelerated_consensus_collator_idx: Option<u32>, //collator index for accelerated consensus mode
}
...
impl RoundState for RoundStateImpl {
    ...
    */
    ...
    fn get_accelerated_consensus_collator_index(&self) -> Option<u32> {
        self.accelerated_consensus_collator_idx
    }

    fn get_node_priority(&self, desc: &dyn SessionDescription, src_idx: u32) -> i32 {
        if let Some(accelerated_consensus_collator_idx) = self.accelerated_consensus_collator_idx {
            if src_idx == accelerated_consensus_collator_idx {
                return 0;
            }
        ...
        desc.get_normal_node_priority(src_idx, self.sequence_number)
    }
    ...
    */
    ...
    fn get_first_attempt(&self, src_idx: u32) -> u32 {
        if self.accelerated_consensus_collator_idx.is_some() {
            unreachable!(
                "RoundState::get_first_attempt: accelerated consensus mode is not supported"
            );
        } else {
    ...
    */
    ...
    fn choose_blocks_to_approve(
        ...
        for block_candidate in self.sent_blocks.get_iter() {
            ...
            let priority = self.get_node_priority(desc, block_source_index);
            ...
            if sources_proposal_set.contains(&block_source_index) {
                ...
                //any honest validator submits at most one block in a round; therefore, we can ignore all blocks from a node if it submits more than one
    ...
    */
    ...
    fn has_voted_block(&self, desc: &dyn SessionDescription) -> bool {
        if self.accelerated_consensus_collator_idx.is_some() {
            //for accelerated consensus mode approved block automatically becomes voted and precommitted
            return self.precommitted_block.is_some();
        }
    ...
    fn choose_block_to_vote(
        ...
        if let Some(accelerated_consensus_collator_idx) = self.accelerated_consensus_collator_idx {
            for block_candidate in self.sent_blocks.get_iter() {
                ...
                assert!(
                    block_candidate.get_source_index() == accelerated_consensus_collator_idx
                        || *block_candidate.get_id() == *SKIP_ROUND_CANDIDATE_BLOCKID
                );

                return Some(block_candidate.get_block().clone());
            }
        }
        ...
        for block_candidate in self.sent_blocks.get_iter() {
            ...
            let priority = match block_candidate.get_block() {
                Some(_block) => self.get_node_priority(desc, block_candidate.get_source_index()),
    ...
    fn generate_vote_for(
        ...
        if self.accelerated_consensus_collator_idx.is_some() {
            unreachable!(
                "RoundState::generate_vote_for: accelerated consensus mode is not supported"
            );
        }
    ...
    */
    ...
    fn get_last_precommit(&self, src_idx: u32) -> u32 {
        if self.accelerated_consensus_collator_idx.is_some() {
            unreachable!(
                "RoundState::get_last_precommit: accelerated consensus mode is not supported"
            );
        }
    ...
    */
    ...
    fn create_action(
        ...
        if self.precommitted_block.is_some() {
            return Some(
                Empty { round: self.get_sequence_number() as i32, attempt: attempt_id as i32 }
                    .into_boxed(),
            );
        }

        //do not generate actions for accelerated consensus mode
    ...
    */
    ...
    fn dump(&self, desc: &dyn SessionDescription) -> String {
        ...
        result = format!(
            "{}    - {}: {} ({:.2}%/{:.2}%), signers: [{}]\n",
            result,
            if sign_weight < desc.get_cutoff_weight() { "unsigned" } else { "signed" },
            sign_weight,
        ...
        if let Some(sent_blocks) = &self.sent_blocks {
            ...
            for block_candidate in sent_blocks.get_iter() {
                ...
                let priority: i32 = if let Some(block) = block {
                    self.get_node_priority(desc, block.get_source_index())
                } else {
                ...
                if let Some(block) = block {
                    result = format!(
                        "{}      - block {:?}: source={}, hash={:?}, root_hash={:x}, file_hash={:?}, {}, priority={}\n",
                        result,
                        block.get_id(),
                    ...
                    result = format!(
                        "{}      - SKIP block: {}, priority={}\n",
                        result, approved_string, priority
                    );
                }
            }
        } else {
        ...
        for i in 0..total_nodes_count {
            if i as usize >= self.first_attempt.len() {
                continue;
            }
        ...
        for i in 0..total_nodes_count {
            if i as usize >= self.last_precommit.len() {
                continue;
            }
...
impl Merge<PoolPtr<dyn RoundState>> for PoolPtr<dyn RoundState> {
    fn merge(&self, right: &Self, desc: &mut dyn SessionDescription) -> Self {
        ...
        assert!(
            left.accelerated_consensus_collator_idx == right.accelerated_consensus_collator_idx
        );
        ...
        let mut precommitted_block = left.precommitted_block.merge(&right.precommitted_block, desc);

        //accelerated consensus mode processing

        if left.accelerated_consensus_collator_idx.is_some() {
            if precommitted_block.is_none() {
                if let Some(new_precommitted_block) =
                    RoundStateImpl::compute_new_precommitted_block_for_accelerated_consensus_mode(
                        desc,
                        left.sequence_number,
                        &sent_blocks,
                    )
                {
                    precommitted_block = Some(new_precommitted_block.get_block().clone());
                }
            }
        }

        let collator_idx = left.accelerated_consensus_collator_idx.clone();
        let (first_attempt, last_precommit, attempts) = if collator_idx.is_some() {
            (None, None, None)
        } else {
            //merge attempts

            let first_attempt = left.first_attempt.merge_custom(
                &right.first_attempt,
                desc,
                &|left, right, _desc| {
                    if *left == 0 {
                ...
                if diversed_voted_attempts_count <= 1 {
                    if let Some(voted_block) = attempt.get_voted_block(desc) {
                        ...
                        if diversed_voted_block_found {
                            diversed_voted_blocks[diversed_voted_attempts_count] =
                                voted_block.clone();
            ...
            (first_attempt, last_precommit, attempts)
        };
        ...
        RoundStateImpl::create(
            ...
            left.get_accelerated_consensus_collator_index(),
            first_attempt,
            last_precommit,
            sent_blocks,
            signatures,
...
impl CacheObject<RoundStateImpl> for RoundStateImpl {
    fn compare(&self, value: &Self) -> bool {
        self.sequence_number == value.sequence_number
            ...
            && self.accelerated_consensus_collator_idx == value.accelerated_consensus_collator_idx
    }
}
...
impl RoundStateImpl {
    ...
    */
    ...
    fn apply_action_dispatch(
        ...
        if self.accelerated_consensus_collator_idx.is_some() {
            //do not process attempts in accelerated consensus mode
        } else {
                ...
                || first_attempt.at(src_idx as usize) > &attempt_id
            {
                ...
                log::trace!(
                    ...
                    attempt_id,
                    self.sequence_number,
                    first_attempt.at(src_idx as usize)
                );
        ...
        if self.accelerated_consensus_collator_idx.is_some() {
            return Self::try_update_precommitted_block_for_accelerated_consensus_mode(state, desc)
                .0;
        }
    ...
    fn apply_submitted_block_action(
        ...
        if self.get_node_priority(desc, src_idx) < 0 {
            log::warn!(
    ...
    fn apply_approved_block_action(
        ...
        if sent_block.is_some() && sent_block.as_ref().unwrap().check_block_is_approved_by(src_idx)
        {
            log::warn!(
        ...
        //accelerated consensus logic: check ONLY ONE block is approved for round (empty or collated block-candidate)

        if self.accelerated_consensus_collator_idx.is_some() {
            for block_candidate in self.sent_blocks.get_iter() {
                if block_candidate.get_id() != &candidate_id
                    && block_candidate.check_block_is_approved_by(src_idx)
                {
                    log::warn!(
                        "Node {} sent an invalid message: another approved block {} has been sent by the same node in accelerated consensus mode: {:?}",
                        desc.get_source_public_key_hash(src_idx),
                        block_candidate.get_id().to_hex_string(),
                        message
                    );
                    return default_state;
                }
        ...
        if sent_block.is_none() {
            ...
            sent_block = Some(SessionFactory::create_unapproved_block_candidate(desc, None));
        }
        ...
        let signature =
            SessionFactory::create_block_candidate_signature(desc, message.signature.to_vec());
    ...
    fn apply_action_on_attempt(
        ...
        //do not apply actions for accelerated consensus mode

        if self.accelerated_consensus_collator_idx.is_some() {
            log::trace!("...skip action on attempt {} for accelerated consensus mode", attempt_id);
            return default_state;
        }
    ...
    fn compute_new_precommitted_block_for_accelerated_consensus_mode(
        desc: &dyn SessionDescription,
        round_sequence_number: u32,
        sent_blocks: &ApproveVector,
    ) -> Option<BlockCandidatePtr> {
        if sent_blocks.is_none() {
            return None;
        }

        let sent_blocks = sent_blocks.as_ref().unwrap();
        let mut approved_blocks = Vec::new();

        for block_candidate in sent_blocks.get_iter() {
            if block_candidate.check_block_is_approved(desc) {
                approved_blocks.push(block_candidate.clone());
            }
        }

        if approved_blocks.len() == 1 {
            return Some(approved_blocks[0].clone());
        } else if approved_blocks.len() > 1 {
            let mut approved_blocks_dump = String::new();

            for block in approved_blocks {
                if !approved_blocks_dump.is_empty() {
                    approved_blocks_dump.push_str(", ");
                }
                approved_blocks_dump.push_str(&format!("{}", block.get_id().to_hex_string()));
            }

            log::warn!(
                "Multiple approved blocks in a round {} for accelerated consensus mode: [{}]",
                round_sequence_number,
                approved_blocks_dump
            );
        }
    ...
    fn try_update_precommitted_block_for_accelerated_consensus_mode(
        state: RoundStatePtr,
        desc: &mut dyn SessionDescription,
    ) -> (RoundStatePtr, bool) {
        ...
        if let Some(new_precommitted_block) =
            Self::compute_new_precommitted_block_for_accelerated_consensus_mode(
                desc,
                state.sequence_number,
                &state.sent_blocks,
            )
        {
            //update state because we have approved block

            log::trace!("......create updated round {} state", state.sequence_number);

            let new_state = Self::create(
                desc,
                Some(new_precommitted_block.get_block().clone()),
                state.sequence_number,
                state.accelerated_consensus_collator_idx,
    ...
    fn make_one_impl(
        ...
        //do not use attempts for accelerated consensus mode

        if desc.is_accelerated_consensus_enabled() {
            return Self::try_update_precommitted_block_for_accelerated_consensus_mode(state, desc);
        }
    ...
    */
    ...
    fn compute_hash(
        ...
        if TELEGRAM_NODE_COMPATIBILITY_HASHES_BUG && accelerated_consensus_collator_idx.is_none() {
    ...
    */
    ...
    fn new(
        ...
        assert!(
            accelerated_consensus_collator_idx.is_none()
                || (first_attempt.is_none() && last_precommit.is_none() && attempts.is_none())
        );
    ...
    pub(crate) fn create_current_round(
        ...
        let (first_attempt, last_precommit) = if desc.is_accelerated_consensus_enabled() {
            (None, None)
        } else {
