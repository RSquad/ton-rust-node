 ...
 */
use crate::{
    BlockHash, BlockId, BlockSignature, CacheEntry, CachedInstanceCounter, CatchainNode, HashType,
    PublicKey, PublicKeyHash, SessionCache, SessionDescription, SessionNode, SessionOptions,
    SessionPool, ValidatorWeight,
};
use catchain::{profiling::ResultStatusCounter, serialize_tl_boxed_object};
use rand::Rng;
use std::{
    collections::HashMap,
    fmt,
...
const MAX_DELAY: std::time::Duration = std::time::Duration::from_secs(100 * 365 * 24 * 60 * 60);
...
impl Default for SessionOptions {
    fn default() -> Self {
        ...
        Self {
            ...
            catchain_max_block_height_coeff: catchain_defaults.max_block_height_coeff,
            catchain_max_serialized_block_size: catchain_defaults.max_serialized_block_size,
            ...
            catchain_receiver_neighbours_sync_min_period: catchain_defaults
                .receiver_neighbours_sync_min_period,
            catchain_receiver_neighbours_sync_max_period: catchain_defaults
                .receiver_neighbours_sync_max_period,
            catchain_receiver_max_sources_sync_attempts: catchain_defaults
                .receiver_max_sources_sync_attempts,
            catchain_receiver_neighbours_rotate_min_period: catchain_defaults
                .receiver_neighbours_rotate_min_period,
            catchain_receiver_neighbours_rotate_max_period: catchain_defaults
                .receiver_neighbours_rotate_max_period,
            round_candidates: 3,
            ...
            proto_version: 0,
            catchain_block_hash_covers_data: false,
            catchain_disable_db: false,
            accelerated_consensus_enabled: false,
            accelerated_consensus_collation_retry_timeout: Duration::from_millis(1000),
            accelerated_consensus_skip_rounds_count_for_collator_rotation: 5,
            accelerated_consensus_max_precollated_blocks: 10,
            validation_retry_attempts: 0,
            validation_retry_timeout: Duration::from_millis(1000),
            block_candidate_sending_retry_timeout: Duration::from_millis(2000),
            block_candidate_sending_retry_attempts: 0,
...
pub(crate) struct SessionDescriptionImpl {
    ...
    reverse_cutoff_weight: ValidatorWeight, //reverse cutoff weight for validators decision making (rejections)
    total_weight: ValidatorWeight,          //total weight of all validators
    self_idx: u32,                          //index of this validator in a list of sources
    accelerated_consensus_initial_collator_index: u32, //accelerated consensus initial collator index
...
impl SessionDescription for SessionDescriptionImpl {
    ...
    */
    ...
    fn get_accelerated_consensus_initial_collator_index(&self) -> u32 {
        self.accelerated_consensus_initial_collator_index
    }
    ...
    */
    ...
    fn get_source_index(&self, public_key_hash: &PublicKeyHash) -> Result<u32> {
        match self.rev_sources.get(public_key_hash) {
            Some(source) => Ok(*source),
            None => Err(error!("Unknown public key hash: {}", public_key_hash)),
        }
    }
    ...
    fn export_full_nodes(&self) -> Vec<PublicKey> {
        self.sources.iter().map(|source| source.public_key.clone()).collect()
    }
    ...
    */
    ...
    fn get_delay(&self, mut priority: u32) -> std::time::Duration {
        if self.options.accelerated_consensus_enabled {
            if priority == 0 {
                return std::time::Duration::from_millis(0);
            }

            if priority == self.get_max_priority() + 1 {
                //special case for empty blocks in accelerated consensus mode
                return self.options.next_candidate_delay;
            }
    ...
    */
    ...
    fn check_signature(
        ...
        self.sources[src_idx as usize].public_key.verify(&serialized_block_id, signature)
    }
...
impl SessionDescriptionImpl {
    ...
    */
    ...
    pub(crate) fn new(
        ...
        accelerated_consensus_initial_collator_index: u32,
        ...
        if options.accelerated_consensus_enabled {
            if options.round_candidates != 1 {
                log::warn!("SessionDescription::new: accelerated_consensus_enabled is set, but round_candidates is not 1; set round_candidates to 1");
        ...
        let reverse_cutoff_weight = total_weight / 3 + 1;
