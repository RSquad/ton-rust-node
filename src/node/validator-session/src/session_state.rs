 ...
 */
use crate::{
    ton, Any, BlockCandidateSignatureVectorPtr, BlockId, CacheObject, CachedInstanceCounter,
    HashType, HashableObject, Merge, MovablePoolObject, OldRoundState, OldRoundStateWrapper,
    ...
    SessionStateWrapper, Vector, VectorMerge, VectorWrapper, SKIP_ROUND_CANDIDATE_BLOCKID,
};
...
impl SessionState for SessionStateImpl {
    ...
    */
    ...
    fn get_current_accelerated_consensus_collator_index(&self) -> Option<u32> {
        self.current_round.get_accelerated_consensus_collator_index()
    }

    fn get_current_round_node_priority(&self, desc: &dyn SessionDescription, src_idx: u32) -> i32 {
        self.current_round.get_node_priority(desc, src_idx)
    }
    ...
    */
    ...
    fn get_current_round_precommitted_block(&self) -> Option<SentBlockPtr> {
        self.current_round.get_precommitted_block()
    }

    fn get_current_round_signatures(&self) -> BlockCandidateSignatureVectorPtr {
        self.current_round.get_signatures().clone()
    }

    fn get_current_round_attempt_number(&self, desc: &dyn SessionDescription) -> u32 {
        let first_attempt = self.current_round.get_first_attempt(desc.get_self_idx());
        let current_attempt = desc.get_attempt_sequence_number(desc.get_ts());

        if current_attempt < first_attempt || first_attempt == 0 {
            return 0;
        }
    ...
    */
    ...
    fn get_block(
        ...
        self.current_round
            .get_block(block_id)
            .map(|block_candidate| block_candidate.get_block().clone())
    }
    ...
    */
    ...
    fn get_committed_block_approve_signatures(
        ...
        if (sequence_number as usize) < self.old_rounds.len() {
            Some(self.old_rounds.at(sequence_number as usize).get_approve_signatures().clone())
        } else {
            None
        }
    }
    ...
    */
    ...
    fn check_need_generate_vote_for(
        ...
        self.current_round.check_need_generate_vote_for(desc, src_idx, attempt)
    }
    ...
    fn generate_vote_for(
        ...
        self.current_round.generate_vote_for(desc, src_idx, attempt_id)
    }
    ...
    */
    ...
    fn get_committed_block_signatures(
        ...
        if (sequence_number as usize) < self.old_rounds.len() {
            Some(self.old_rounds.at(sequence_number as usize).get_signatures().clone())
        } else {
            None
        }
    }
    ...
    */
    ...
    fn apply_action(
        ...
        if round_id > current_round_id {
            ...
            log::warn!("...too big round ID {} (current round is {})", round_id, current_round_id);
        ...
        if round_id == current_round_id {
            ...
            if new_current_round.check_block_is_signed(desc) {
                ...
                let accelerated_consensus_next_round_collator_idx =
                    Self::compute_accelerated_consensus_next_collator_index(
                        desc,
                        new_current_round.get_accelerated_consensus_collator_index(),
                        &new_old_rounds,
                    );

                new_current_round = SessionFactory::create_round(
                    desc,
                    new_current_round.get_sequence_number() + 1,
                    accelerated_consensus_next_round_collator_idx,
                );
            }
            ...
            let new_old_rounds = self.old_rounds.change(desc, round_id as usize, new_old_round);

            Self::create(desc, new_attempt_ids, self.current_round.clone(), new_old_rounds)
        }
    }
...
impl Merge<PoolPtr<dyn SessionState>> for PoolPtr<dyn SessionState> {
    fn merge(&self, right: &Self, desc: &mut dyn SessionDescription) -> Self {
        ...
        let attempt_ids =
            left.attempt_ids.merge_custom(&right.attempt_ids, desc, &|left, right, _desc| {
        ...
        if round.check_block_is_signed(desc) {
            ...
            let accelerated_consensus_next_round_collator_idx =
                SessionStateImpl::compute_accelerated_consensus_next_collator_index(
                    desc,
                    round.get_accelerated_consensus_collator_index(),
                    &old_rounds,
                );

            round = SessionFactory::create_round(
                desc,
                round.get_sequence_number() + 1,
                accelerated_consensus_next_round_collator_idx,
            );
            //create new round
        }
...
pub(crate) fn get_impl(value: &dyn SessionState) -> &SessionStateImpl {
...
impl SessionStateImpl {
    ...
    */
    ...
    fn compute_accelerated_consensus_next_collator_index(
        desc: &dyn SessionDescription,
        current_collator_idx: Option<u32>,
        ...
        if let Some(old_rounds) = old_rounds {
            let mut skip_rounds_count_for_collator_rotation: i32 =
                desc.opts().accelerated_consensus_skip_rounds_count_for_collator_rotation as i32;

            if skip_rounds_count_for_collator_rotation == 0 {
                //special case: infinite collator without rotation
                return current_collator_idx;
            }

            for old_round in old_rounds.iter().rev() {
                if *old_round.get_block_id() != *SKIP_ROUND_CANDIDATE_BLOCKID {
                    return current_collator_idx;
                }

                if current_collator_idx != old_round.get_accelerated_consensus_collator_index() {
                    return current_collator_idx;
                }

                skip_rounds_count_for_collator_rotation -= 1;

                if skip_rounds_count_for_collator_rotation == 0 {
                    //rotate collator
                    return match current_collator_idx {
                        Some(idx) => Some((idx + 1) % desc.get_total_nodes() as u32),
                        _ => None,
                    };
                }
            }

            assert!(skip_rounds_count_for_collator_rotation > 0);
        }

        current_collator_idx
    }
    ...
    */
    ...
    pub(crate) fn make_one(
        ...
        log::trace!("......actualizing state (source={}, attempt={})", src_idx, attempt_id);
    ...
    */
    ...
    pub(crate) fn create_empty(desc: &mut dyn SessionDescription) -> SessionStatePtr {
        ...
        let accelerated_consensus_collator_idx = if desc.is_accelerated_consensus_enabled() {
            Some(desc.get_accelerated_consensus_initial_collator_index())
        } else {
            None
        };
        let current_round =
            SessionFactory::create_round(desc, 0, accelerated_consensus_collator_idx);
