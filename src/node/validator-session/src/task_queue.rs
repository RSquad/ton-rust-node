    ...
    F: FnOnce(Result<T>, &mut dyn SessionProcessor) + 'static,
{
    ...
    let queue_weak_ptr =
        Arc::downgrade(&completion_handler_processor.get_completion_task_queue().clone());
        ...
        if let Some(queue) = queue_weak_ptr.upgrade() {
            post_closure(&queue, move |processor: &mut dyn SessionProcessor| {
                if let Some(mut handler) = processor.remove_completion_handler(handler_index) {
                    if let Some(handler) =
                        handler.get_mut_impl().downcast_mut::<SingleThreadedCompletionHandler<T>>()
                    {
...
impl<T> SingleThreadedCompletionHandler<T> {
    fn new(handler: SessionProcessorCallback<T>) -> Self {
        Self { handler: Some(handler), creation_time: std::time::SystemTime::now() }
    }
}
