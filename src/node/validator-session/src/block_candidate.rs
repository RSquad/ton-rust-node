 ...
 */
use crate::{
    ton, Any, BlockCandidate, BlockCandidatePtr, BlockCandidateSignature,
    BlockCandidateSignaturePtr, BlockCandidateSignatureVectorPtr, BlockCandidateWrapper, BlockId,
    BlockSignature, CacheObject, CachedInstanceCounter, HashType, HashableObject, Merge,
    MovablePoolObject, PoolObject, PoolPtr, SentBlockPtr, SessionCache, SessionDescription,
    SessionFactory, SessionPool, VectorMerge, DEFAULT_BLOCKID,
};
...
impl fmt::Display for BlockCandidateSignatureImpl {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "BlockCandidateSignature(hash={:?}, signature={:?})", &self.hash, &self.signature)
    }
}
...
impl BlockCandidateSignatureImpl {
    ...
    */
    ...
    pub(crate) fn create(
        ...
        let body = Self::new(signature, desc.get_block_candidate_signatures_instance_counter());
...
impl BlockCandidate for BlockCandidateImpl {
    ...
    */
    ...
    fn check_block_is_approved(&self, desc: &dyn SessionDescription) -> bool {
        let block_approval_cutoff_weight =
            if desc.is_accelerated_consensus_enabled() && self.block.is_none() {
                //special case for empty block for accelerated consensus mode
                //  - assume only one block is approved for round in accelerated consensus mode (empty or collated block-candidate)
                //  - if 1/3 of validators plus one approve empty block, a block-candidate can't be approved, so round should be skipped
                //  => for this special case in accelerated consensus mode, we approve empty block candidate by 1/3 of weights (not 2/3 as usual)
                desc.get_reverse_cutoff_weight()
            } else {
                desc.get_cutoff_weight()
            };
        ...
        for i in 0..desc.get_total_nodes() {
            if self.approved_by.at(i as usize).is_some() {
                ...
                if weight >= block_approval_cutoff_weight {
                    return true;
                }
            }
        }
...
impl Merge<PoolPtr<dyn BlockCandidate>> for PoolPtr<dyn BlockCandidate> {
    fn merge(&self, right: &Self, desc: &mut dyn SessionDescription) -> Self {
        ...
        let approved_by = left.get_approvers_list().merge(right.get_approvers_list(), desc);
...
impl BlockCandidateWrapper for BlockCandidatePtr {
    fn push(
        ...
        let approved_by = self_impl.approved_by.change(desc, src_idx as usize, signature);
...
impl fmt::Display for BlockCandidateImpl {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "BlockCandidate(hash={:08x}, block={:?})", &self.hash, &self.block)
    }
}
...
fn get_impl(value: &dyn BlockCandidate) -> &BlockCandidateImpl {
    value.get_impl().downcast_ref::<BlockCandidateImpl>().unwrap()
}
...
impl BlockCandidateImpl {
    ...
    */
    ...
    pub(crate) fn create(
        ...
        let body = Self::new(block, approved_by, desc.get_block_candidates_instance_counter());
