 ...
 */
use crate::{
    ton, Any, BlockId, BoolVectorPtr, CacheObject, CachedInstanceCounter, HashType, HashableObject,
    Merge, MovablePoolObject, PoolObject, PoolPtr, RoundAttemptState, RoundAttemptStatePtr,
    RoundAttemptStateWrapper, RoundState, SentBlockPtr, SentBlockWrapper, SessionCache,
    SessionDescription, SessionFactory, SessionPool, SortedVectorWrapper, ValidatorWeight,
    VectorMerge, VoteCandidatePtr, VoteCandidateVectorPtr, VoteCandidateWrapper,
...
impl RoundAttemptState for RoundAttemptStateImpl {
    ...
    */
    ...
    fn create_action(
        ...
        log::trace!("...create attempt action for source={} and attempt={}", src_idx, attempt_id);
        ...
        if !self.check_precommit_received_from(src_idx) {
            ...
            if let Some(voted_block) = self.get_voted_block(desc) {
                ...
                log::trace!("...block {:?} has been choosen to precommit", voted_block_id);
    ...
    */
    ...
    fn dump(&self, desc: &dyn SessionDescription) -> String {
        ...
        if let Some(ref votes) = self.votes {
            ...
            result = format!("{}        - votes ({} candidates):\n", result, votes_map.len());
...
fn get_impl(value: &dyn RoundAttemptState) -> &RoundAttemptStateImpl {
    value.get_impl().downcast_ref::<RoundAttemptStateImpl>().unwrap()
}
    ...
    */
    ...
    fn apply_action_dispatch(
        ...
        match message {
            ...
            _ => {
                unreachable!("Unknown message appears for attempt processing: {:?}", message);
            }
        }
    }
    ...
    fn apply_vote_for_action(
        ...
        if src_idx != desc.get_vote_for_author(attempt_id) {
            log::warn!(
                ...
                desc.get_source_public_key_hash(src_idx),
                desc.get_vote_for_author(attempt_id),
                src_idx,
                attempt_id,
                message
            );
        ...
        if vote_for_block.is_none()
            || !vote_for_block.as_ref().unwrap().check_block_is_approved(desc)
        {
            log::warn!(
    ...
    fn apply_default_action(
        ...
        let vote_result =
            self.try_vote(desc, src_idx, attempt_id, message, round_state, default_state.clone());
