 ...
 */
use crate::{
    utils, Any, BoolVector, CacheObject, CachedInstanceCounter, HashType, HashableObject,
    MovablePoolObject, PoolObject, PoolPtr, SessionCache, SessionDescription, SessionPool,
    TypeDesc, VectorMerge,
};
...
impl BoolVectorImpl {
    ...
    fn with_data(data: Vec<bool>, instance_counter: &CachedInstanceCounter) -> Self {
        let elements_count =
            if data.len() % 32 == 0 { data.len() } else { data.len() - data.len() % 32 + 32 };
