...
use crate::{
    ...
    Merge, MovablePoolObject, PoolObject, PoolPtr, SentBlockPtr, SentBlockWrapper, SessionCache,
    SessionDescription, SessionFactory, SessionPool, VectorMerge, VoteCandidate, VoteCandidatePtr,
    VoteCandidateWrapper,
};
...
impl Merge<PoolPtr<dyn VoteCandidate>> for PoolPtr<dyn VoteCandidate> {
    fn merge(&self, right: &Self, desc: &mut dyn SessionDescription) -> Self {
        ...
        let voted_by = get_impl(&**left).voted_by.merge(&get_impl(&**right).voted_by, desc);
...
fn get_impl(value: &dyn VoteCandidate) -> &VoteCandidateImpl {
    value.get_impl().downcast_ref::<VoteCandidateImpl>().unwrap()
}
