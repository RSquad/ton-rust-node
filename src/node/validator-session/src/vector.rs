 ...
 */
use crate::{
    ton, Any, CacheObject, CachedInstanceCounter, HashType, HashableObject, MovablePoolObject,
    PoolObject, PoolPtr, SessionCache, SessionDescription, SessionPool, SortedVector,
    ...
    T: HashableObject,
{
    ...
    crate::utils::compute_hash(ton::hashable::Vector { value: hashes })
}
        ...
        + 'static,
{
    ...
    */
    ...
    fn modify(
        ...
        let modified_vec = self.data.clone().into_iter().map(|x| modifier(&x)).collect();
    ...
    */
    ...
    fn clone_to_persistent(&self, cache: &mut dyn SessionCache) -> PoolPtr<dyn Vector<T>> {
        ...
        let data_cloned = self.data.iter().map(|x| x.move_to_persistent(cache)).collect();
        ...
        match &self {
            Some(ref src) => {
                ...
                if data.len() > right as usize {
                    result.data.extend_from_slice(&data[right as usize..data.len()]);
                }

                result.recompute_hash();

                Some(SortedVectorImpl::<T>::create_temp_object(result, desc.get_cache()))
            }
            _ => {
                let mut result =
