...
impl FullNodeNetworkTelemetry {
    ...
    pub fn consumed_query(&self, query: String, success: bool, time: Duration, data: usize) {
        let time = time.as_millis() as u64;
        adnl::common::add_unbound_object_to_map_with_update(&self.queries, query, |found| {
    ...
    pub fn report(&self, update_period_sec: u64) -> String {
        ...
        report
            .string()
            .expect("unexpected error while building full node service's telemetry report")
    }
}
