...
pub struct FullNodeTelemetry {
    ...
    sent_block_candidate_broadcasts: AtomicU64,
...
impl Default for FullNodeTelemetry {
    fn default() -> Self {
        Self {
            ...
            sent_block_candidate_broadcasts: AtomicU64::new(0),
...
impl FullNodeTelemetry {
    ...
    pub fn good_top_block_broadcast(&self, block_id: &BlockIdExt) {
        ...
        if add_unbound_object_to_map(&self.last_top_block_broadcasts, block_id.clone(), || {
            Ok(Instant::now())
        })
    ...
    #[allow(dead_code)]
    pub fn sent_block_candidate_broadcast(&self) {
        self.sent_block_candidate_broadcasts.fetch_add(1, Ordering::Relaxed);
    }
    ...
    pub fn report(&self, tps_1: u32, tps_2: u32) -> String {
        ...
        let block_broadcasts_duplicates =
            self.block_broadcasts_duplicates.swap(0, Ordering::Relaxed);
        ...
        let block_download_time_min =
            self.block_download_time_min.swap(u64::MAX, Ordering::Relaxed);
        ...
        let block_broadcast_delay_min =
            self.block_broadcast_delay_min.swap(u64::MAX, Ordering::Relaxed);
        ...
        let sent_block_candidate_broadcasts =
            self.sent_block_candidate_broadcasts.swap(0, Ordering::Relaxed);
        ...
        report.append(format!(
            "applied blocks              {:>10} {:>3.0}%\n",
            applied_blocks,
            if pre_applied_blocks > 0 {
                applied_blocks as f64 / pre_applied_blocks as f64 * 100_f64
            } else {
                0_f64
            }
        ));
        ...
        report.append(format!(
            "    got by broadcast        {:>10} {:>3.0}%\n",
            pre_applied_blocks_bcast,
            if pre_applied_blocks > 0 {
                pre_applied_blocks_bcast as f64 / pre_applied_blocks as f64 * 100_f64
            } else {
                0_f64
            }
        ));
        report.append(format!(
            ...
            if pre_applied_blocks > 0 {
                pre_applied_blocks_dloaded as f64 / pre_applied_blocks as f64 * 100_f64
            } else {
                0_f64
            }
        ));
        ...
        report.append(format!(
            "    block candidates        {:>10}\n",
            sent_block_candidate_broadcasts
        ));
        report.append("***\n");
        report.append(format!("bad top block broadcast     {:>10}\n", top_block_broadcasts_bad));
        report.append(format!(
            "good top block broadcast    {:>10} 100%\n",
            top_block_broadcasts_unic
        ));
        report.append(format!(
            "top block broadcast dupl    {:>10} {:>3.0}%\n",
            top_block_broadcasts_dupl,
            if top_block_broadcasts_dupl != 0 {
                top_block_broadcasts_dupl as f64 / top_block_broadcasts_unic as f64 * 100_f64
            } else {
                0_f64
            }
        ));
        ...
        report.append(format!(
            "block broadcast duplicates  {:>10} {:>3.0}%\n",
            block_broadcasts_duplicates,
            if block_broadcasts > 0 {
                block_broadcasts_duplicates as f64 / block_broadcasts as f64 * 100_f64
            } else {
                0_f64
            }
        ));
        report.append(format!(
            "unneeded block-broadcast    {:>10} {:>3.0}%\n",
            block_broadcasts_unneeded,
            if block_broadcasts > 0 {
                block_broadcasts_unneeded as f64 / block_broadcasts as f64 * 100_f64
            } else {
                0_f64
            }
        ));
        ...
        for (i, attempts) in block_downloaded_attempts.iter().enumerate() {
            ...
            report.append(format!(
                ...
                attempts,
                if block_downloaded_attempts_sum > 0 {
                    *attempts as f64 / block_downloaded_attempts_sum as f64 * 100_f64
                } else {
                    0_f64
                }
            ));
