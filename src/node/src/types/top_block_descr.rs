...
use crate::{
    ...
    validating_utils::{build_checked_data, UNREGISTERED_CHAIN_MAX_LEN},
    validator::validator_utils::{calc_subset_for_workchain_standard, check_crypto_signatures},
};
use std::{
    ...
    sync::{
        atomic::{self, AtomicI8},
        Arc,
    },
};
use ton_api::ton::ton_node::newshardblock::NewShardBlock;
use ton_block::{
    error, fail, AddSub, Block, BlockIdExt, BlockInfo, BlockSignaturesVariant, BocReader,
    BocWriter, Cell, CurrencyCollection, Deserializable, HashmapAugType, McShardRecord,
...
impl TopBlockDescrStuff {
    pub fn new(
        ...
        if tbd.chain().len() > UNREGISTERED_CHAIN_MAX_LEN as usize {
            fail!(
                "invalid ShardTopBlockDescr for {}: its proof-chain is too long ({} > {})",
                tbd.proof_for(),
                tbd.chain().len(),
    ...
    fn read_one_proof(
        ...
        let (_, mut prev_stuff) = construct_and_check_prev_stuff(&block_virt_root, cur_id, false)
            .map_err(|e| {
            error!(
        ...
        let prev2_id =
            if prev_stuff.prev.len() > 1 { Some(prev_stuff.prev.pop().unwrap()) } else { None };
        ...
        if let Some(signatures) = signatures {
            let validator_info = signatures.validator_info();
            if info.gen_catchain_seqno() != validator_info.catchain_seqno {
                fail!(
            ...
            if info.gen_validator_list_hash_short() != validator_info.validator_list_hash_short {
                fail!(
                    "link for block {} is invalid because block header \
                    ...
                    validator_info.validator_list_hash_short
                )
            }
        }
        ...
        */
        ...
        if let Some((next_id, next_mc_id, next_info)) = next_info {
            ...
            if info.gen_utime() > next_info.gen_utime() {
                fail!(
    ...
    fn validate_internal(
        ...
        for mc_id in self.chain_mc_blk_ids.iter() {
            ...
            if !valid {
                *res_flags |= 1; // permanently invalid
                fail!(
                    "ShardTopBlockDescr for {} is invalid because \
                    it refers to masterchain block {mc_id} \
                    which is not an ancestor of our block {last_mc_block_id}",
                    self.proof_for(),
                )
            }
        }
        ...
        let l_shard_descr =
            last_mc_state_extra.shards().find_shard(&l_shard_id)?.ok_or_else(|| {
                error!(
        ...
        let r_shard_descr = if !self.proof_for().shard().is_parent_for(l_shard_descr.shard()) {
            ...
            debug_assert!(l_shard_descr
                .block_id()
                .shard()
                .is_ancestor_for(self.proof_for().shard()));
            l_shard_descr.clone()
        } else {
            ...
            let r_shard_descr =
                last_mc_state_extra.shards().find_shard(&r_shard_id)?.ok_or_else(|| {
                    error!(
                        "ShardTopBlockDescr for {} is invalid or too new because this workchain \
        ...
        let clen = (self.proof_for().seq_no
            - max(l_shard_descr.block_id().seq_no(), r_shard_descr.block_id().seq_no()))
            as usize;
        debug_assert!(clen > 0);
        ...
        if clen < self.size() {
            if &self.chain_blk_ids[clen] != l_shard_descr.block_id() {
                *res_flags |= 1;
                fail!(
                    "ShardTopBlockDescr for {} is invalid: it contains a reference to \
                    its ancestor {} but the masterchain refers to \
                    another shardchain block {} of the same height",
                    self.proof_for(),
            ...
            if &self.chain_head_prev[0] != l_shard_descr.block_id() {
                *res_flags |= 1;
                fail!(
                    "ShardTopBlockDescr for {} is invalid: it contains a reference to \
                    its left ancestor {} but the masterchain instead refers to \
                    another shardchain block {}",
                    self.proof_for(),
                    self.chain_head_prev[0],
                    l_shard_descr.block_id()
                )
            ...
            if chain_head_prev2 != r_shard_descr.block_id() {
                *res_flags |= 1;
                fail!(
                    "ShardTopBlockDescr for {} is invalid: it contains a reference to \
                    its right ancestor {} but the masterchain instead refers to \
                    another shardchain block {}",
                    self.proof_for(),
                    chain_head_prev2,
                    r_shard_descr.block_id()
        ...
        let cc_seqno =
            last_mc_state_extra.shards().calc_shard_cc_seqno(self.proof_for().shard())?;
        ...
        // Get signatures using variant-aware accessor (supports both Ordinary and Simplex)
        let signatures_variant = self
            .tbd
            .signatures()
            .ok_or_else(|| error!("There is no signatures in top block descr"))?;

        let validator_info = signatures_variant.validator_info();

        if (cc_seqno == validator_info.catchain_seqno)
            && (subset.short_hash == validator_info.validator_list_hash_short)
        {
            *res_flags |= 4;
            vset_ok = true;
            ...
            if (cc_seqno == validator_info.catchain_seqno)
                && (next_subset.short_hash == validator_info.validator_list_hash_short)
            {
                *res_flags |= 8;
                vset_ok = true;
                subset = next_subset;
        ...
        match self.signarutes_validation_result.load(atomic::Ordering::Relaxed) {
            -1 => fail!(
                "ShardTopBlockDescr for {} has bad signatures (according to cached result)",
                self.proof_for()
            ),
            1 => {
        ...
        // Generate checked_data based on signature variant (C++ polymorphic to_sign())
        let checked_data =
            build_checked_data(signatures_variant, self.proof_for()).map_err(|err| {
                error!(
                    "ShardTopBlockDescr for {}: failed to build checked_data: {}",
                    self.proof_for(),
                    err
                )
            })?;

        let sig_type = match signatures_variant {
            BlockSignaturesVariant::Ordinary(_) => "ORDINARY",
            BlockSignaturesVariant::Simplex(_) => "SIMPLEX",
        };
        log::trace!(
            target: "validator",
            "TopBlockDescrStuff::validate_internal: checking {} signatures for {}",
            sig_type,
            self.proof_for()
        );

        let total_weight: u64 = subset.validators.iter().map(|v| v.weight).sum();
        let weight = match check_crypto_signatures(
            signatures_variant.pure_signatures(),
            &subset.validators,
            &checked_data,
        ) {
            Err(err) => {
                log::warn!(
                    target: "validator",
                    "TopBlockDescrStuff::validate_internal: {} signature verification failed for {}: {}",
                    sig_type,
                    self.proof_for(),
                    err
                );
                *res_flags |= 0x21;
                ...
                fail!(
                    "ShardTopBlockDescr for {} does not have valid signatures: {err}",
                    self.proof_for()
                )
            }
            Ok(w) => w,
        ...
        if !self.is_fake && weight * 3 <= total_weight * 2 {
            *res_flags |= 0x21;
            self.signarutes_validation_result.store(-1, atomic::Ordering::Relaxed);
            fail!(
                "ShardTopBlockDescr for {} has too small signatures weight {weight} of {total_weight}",
                self.proof_for()
            );
        }
        *res_flags |= 0x10; // signatures checked ok
        ...
        // Check weight
        if !self.is_fake && weight != signatures_variant.pure_signatures().weight() {
            self.signarutes_validation_result.store(-1, atomic::Ordering::Relaxed);
            fail!(
                "ShardTopBlockDescr for {} has incorrect signature weight {} \
                (actual weight is {weight})",
                self.proof_for(),
                signatures_variant.pure_signatures().weight()
            );
        }
        ...
        log::debug!(
            "ShardTopBlockDescr for {} has valid validator signatures \
            of total weight {weight} out of {total_weight}",
            self.proof_for()
        );
    ...
    match a.shard().workchain_id().cmp(&b.shard().workchain_id()) {
        Ordering::Equal => {
            match a.shard().shard_prefix_with_tag().cmp(&b.shard().shard_prefix_with_tag()) {
                Ordering::Equal => b.seq_no().cmp(&a.seq_no()),
                ord => ord,
            }
        }
        ord => ord,
...
mod id {
    ...
    impl TopBlockDescrGroup {
        ...
        pub fn new_id(self, seqno: u32) -> TopBlockDescrId {
    ...
    impl TopBlockDescrId {
        pub fn new(shard: ShardIdent, cc_seqno: u32, seqno: u32) -> Self {
            Self { group: TopBlockDescrGroup::new(shard, cc_seqno), seqno }
        }
        ...
        pub fn from_bytes(bytes: &[u8]) -> Result<Self> {
            ...
            Ok(Self { group, seqno: u32::from_le_bytes(seqno.try_into()?) })
        }
...
fn get_ref_shard_blocks(tbd: &TopBlockDescr) -> Result<Vec<(BlockIdExt, u64)>> {
    ...
    let extra = block.read_extra()?;
    extra.read_wc_custom()?.ref_shard_blocks.iterate_shard_block_refs(|id, end_lt| {
