...
use adnl::{
    common::{
        add_counted_object_to_map, add_counted_object_to_map_with_update, CountedObject, Counter,
    },
    declare_counted,
};
use rand::Rng;
...
use ton_block::{fail, BlockIdExt, BlockSignatures, Deserializable, Result, TopBlockDescr};
...
impl ShardBlocksPool {
    pub fn new(
        ...
        for (key, val) in shard_blocks {
            let val = Arc::new(val);
            add_counted_object_to_map(&tsbs, key, || {
        ...
        let tbds = loop {
            ...
            let added = add_counted_object_to_map_with_update(
                ...
                |found| {
                    ...
                    #[cfg(feature = "telemetry")]
                    engine.engine_telemetry().top_blocks.update(top_blocks.load(Ordering::Relaxed));
                    Ok(Some(ret))
                },
            )?;
            if !added {
    ...
    pub async fn update_shard_blocks(&self, last_mc_state: &Arc<ShardStateStuff>) -> Result<()> {
        ...
        log::trace!(
            "update_shard_blocks last_mc_state {} removed: {}",
