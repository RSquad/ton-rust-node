...
use ton_block::{
    ...
    LabelReader, OutMsgQueue, Result, SliceData,
};
...
impl HashmapNodeObj {
    ...
    pub fn parse_node_from_cell(
        ...
        let lt = cursor.get_int(64)?;

        let node_obj = HashmapNodeObj { key, remainder: cursor, lt };

        Ok(HashmapNodeParseResult { node_obj, bottom_bit_len: current_bit_len })
    }
}
...
impl HashmapNode {
    ...
    fn extract_node_cell(&mut self) -> Result<Cell> {
...
impl HashmapOrderedFilterCursor {
        ...
        F: FnMut(&HashmapNodeObj) -> Result<HashmapFilterResult>,
    {
        ...
        if self.process_executed {
            Err(error!("unable to process hashmap again with current cursor"))
        } else {
        ...
        F: FnMut(&HashmapNodeObj) -> Result<HashmapFilterResult>,
    {
        if self.ordered_queue.is_empty() {
            log::debug!("clean_out_msg_queue: hop {}: queue is empty", self.hops_counter);
        ...
        F: FnMut(&HashmapNodeObj) -> Result<HashmapFilterResult>,
    {
        ...
        match filter_func(current.node_obj_ref())? {
            ...
            HashmapFilterResult::Stop => {
                log::trace!("clean_out_msg_queue: hop {}: filter result = Stop", self.hops_counter);
                self.stop_processing = true;
            }
    ...
    fn handle_child_result(
        ...
        if let Some(current) = current {
            ...
            if processed {
                ...
                handle_child_elapsed += handle_child_timer.elapsed().as_nanos() as i128;
                self.sum_avg_time(CursorTimerType::HandleChildResult, handle_child_elapsed, false);
    ...
    fn aggregate_children_results(
        ...
        // convert fork into edge if only one child removed
            ...
            let child_opt = if current.child_left_removed { child_right } else { child_left };
        ...
        // update current fork if both children not removed but at least one updated
            ...
            let (builder, _) = OutMsgQueue::make_fork(
                &label,
                top_bit_len,
                child_left_cell,
                child_right_cell,
