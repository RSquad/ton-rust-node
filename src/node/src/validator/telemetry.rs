...
impl CollatorValidatorTelemetry {
    pub fn succeeded_attempt(
        ...
        if shard.is_masterchain() {
            ...
            add_unbound_object_to_map_with_update(&self.shardes, shard.clone(), |found| {
    ...
    pub fn report(&self) -> String {
        ...
        report
            .string()
            .expect("unexpected error while building collator/validator telemetry report")
    }
}
...
impl ShardTelemetry {
    pub fn succeeded_attempt(&self, time: Duration, transactions: u32, gas: u32) {
        ...
        self.transactions_per_block
            [min((transactions / TR_PER_BLOCK_STEP) as usize, TR_PER_BLOCK_STEPS - 1)]
...
impl ShardTelemetrySample {
    ...
    pub fn report(&self) -> String {
        ...
        report.append(format!(
            "total succeeded           {:>10}  {:>3.0}%\n",
            total_success,
            total_success as f64 / self.total_attempts as f64 * 100_f64
        ));
        report.append(format!(
            "longer than {:>4}ms        {:>10}  {:>3.0}%\n",
        ...
        report.append(format!(
            "total failed              {:>10}  {:>3.0}%\n",
            self.total_fail,
            self.total_fail as f64 / self.total_attempts as f64 * 100_f64
        ));

        if self.total_fail > 0 {
            report.append("reasons of fail:\n");
            report.append(format!(
                "    no wait state         {:>10}  {:>3.0}%\n",
                self.wait_state_fail,
                self.wait_state_fail as f64 / self.total_fail as f64 * 100_f64
            ));
            report.append(format!(
                "    can't get_block       {:>10}  {:>3.0}%\n",
                self.get_block_fail,
                self.get_block_fail as f64 / self.total_fail as f64 * 100_f64
            ));
            report.append(format!(
                "    wrong validator set   {:>10}  {:>3.0}%\n",
                self.wrong_validator_set,
                self.wrong_validator_set as f64 / self.total_fail as f64 * 100_f64
            ));
            report.append(format!(
                "    8 blocks w/a mc commit{:>10}  {:>3.0}%\n",
                self.more_than_8_blocks,
                self.more_than_8_blocks as f64 / self.total_fail as f64 * 100_f64
            ));
            report.append(format!(
                "    given mc isn't actual {:>10}  {:>3.0}%\n",
                self.not_actual_mc,
                self.not_actual_mc as f64 / self.total_fail as f64 * 100_f64
            ));
            let other_fail = self.total_fail
                - self.wait_state_fail
                - self.get_block_fail
                - self.wrong_validator_set
                - self.more_than_8_blocks
            ...
            report.append(format!(
                "    other                 {:>10}  {:>3.0}%\n",
                other_fail,
                other_fail as f64 / self.total_fail as f64 * 100_f64
            ));
        }
