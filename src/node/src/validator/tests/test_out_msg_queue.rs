...
// put it to its queue
// then perform hyper routing to another shard
...
fn test_already_processed_enqueued_message() {
    ...
    let mut last_entry =
        ProcessedUptoStuff::with_params(0x9800000000000000, 73107, last_msg_lt, last_msg_hash);
    ...
    let mut fake_entry =
        ProcessedUptoStuff::with_params(0x9800000000000000, 73102, fake_msg_lt, fake_msg_hash);
...
fn test_contains_processed_up_to() {
    ...
    let put1 = ProcessedUptoStuff::with_params(0x40, 4, 8_000_007, [0xff; 32].into());
    let put2 = ProcessedUptoStuff::with_params(0xC0, 5, 9_000_007, [0xff; 32].into());
    // and shard is merge result
    let put3 = ProcessedUptoStuff::with_params(0x80, 6, 9_000_007, [0xff; 32].into());
    ...
    let put1 = ProcessedUptoStuff::with_params(SHARD_FULL, 5, 9_000_007, [0x11; 32].into());
    let put2 = ProcessedUptoStuff::with_params(SHARD_FULL, 5, 9_000_007, [0xff; 32].into());
    let put3 = ProcessedUptoStuff::with_params(SHARD_FULL, 6, 11_000_007, [0xff; 32].into());
    assert!(put2.contains(&put1));
    assert!(!put1.contains(&put2));
    assert!(put3.contains(&put1));
    assert!(!put1.contains(&put3));
    ...
    let mut put_old = ProcessedUptoStuff::with_params(SHARD_FULL, 5, 9_000_007, [0xff; 32].into());
    let mut put_new = ProcessedUptoStuff::with_params(SHARD_FULL, 6, 11_000_007, [0xff; 32].into());
    ...
    let mut put_old = ProcessedUptoStuff::with_params(SHARD_FULL, 2, 3_000_012, [0xff; 32].into());
    let mut put_new = ProcessedUptoStuff::with_params(SHARD_FULL, 3, 5_000_007, enq.message_hash());
...
impl NewMessageTest {
    pub fn new(
        ...
        Self { lt_hash, msg, tr_cell, prefix }
    }
}
...
pub fn generate_test_queue(
    ...
    println!("Queue length: {}, enqueue count: {}", max_lt, enqueue_lt);
    ...
    for lt in 1..=max_lt {
        ...
        let prefix = address.get_int(64).unwrap();
        let account_id_prefix = AccountIdPrefixFull { workchain_id: 0, prefix };
    ...
    */
    ...
    fn clean_out_msg_queue_ordered(
        ...
        for k_v in iter {
            ...
            if log_msgs_order {
                log_num += 1;
                println!("{log_num}: lt {}, {}", enq.lt(), enq.message().dst().unwrap());
            }
    ...
    fn clean_out_msg_queue_combined(
        ...
        let clean_timeout_nanos =
            (stop_timeout_ms as i128) * 1_000_000 * (clean_timeout_percentage_points as i128)
                / 1000;

        let fast_filtering_timeout_nanos =
        ...
        if partial {
            ...
            let fast_filtering_elapsed_nanos = timer.elapsed().as_nanos() as i128;
            let ordered_cleaning_timeout_nanos = clean_timeout_nanos - fast_filtering_elapsed_nanos;
            println!(
    ...
    fn process_new_messages(
        ...
        for NewMessageTest { lt_hash: (_created_lt, _hash), msg, tr_cell, prefix: _ } in
            new_messages.drain()
        {
            let info = msg.int_header().ok_or_else(|| error!("message is not internal")).unwrap();
            let fwd_fee = *info.fwd_fee();
            let msg_cell = msg.serialize()?;
            let enq =
                MsgEnqueueStuff::new(msg, msg_cell, shard, fwd_fee, false, Default::default())
                    .unwrap();
    ...
    fn process_new_messages_without_drain(
        ...
        while let Some(NewMessageTest { lt_hash: (_created_lt, _hash), msg, tr_cell, prefix: _ }) =
            new_messages.pop()
        {
    ...
    fn split_compare(mut queue: OutMsgQueue, shard: &ShardIdent) -> Result<()> {
        ...
        println!("Shard left length: {}", left.len()?);
        Ok(())
    }
    ...
    check_multi(10, "Splitting queue creating left", || split_creating_shard(queue.clone(), &s0));
    check_multi(10, "Splitting queue creating right", || split_creating_shard(queue.clone(), &s1));
    ...
    check_multi(10, "Splitting queue filtering to left and creating remaining", || {
        split_filtering_to_shard_and_creating_remaining(queue.clone(), &s0)
    });
}

#[test]
fn test_build_out_msgs_proofs() -> Result<()> {
    crate::test_helper::init_test_log();

    let shards = [
        ShardIdent::with_tagged_prefix(0, 0x2000_0000_0000_0000)?,
        ShardIdent::with_tagged_prefix(0, 0x6000_0000_0000_0000)?,
        ShardIdent::with_tagged_prefix(0, 0xa000_0000_0000_0000)?,
        ShardIdent::with_tagged_prefix(0, 0xd000_0000_0000_0000)?,
        ShardIdent::with_tagged_prefix(0, 0xf000_0000_0000_0000)?,
    ];
    ...
    for shard in &shards {
        ...
        let (queue, _) = generate_test_queue(2 * 1024, 0, vec![]);
        let oqi = OutMsgQueueInfo::with_params(
            queue,
            ProcessedInfo::default(),
            OutMsgQueueExtra::default(),
        );
        let mut ss = ShardStateUnsplit::default();
        ss.write_out_msg_queue_info(&oqi)?;
        state_roots.push(ss.serialize()?);
    }

    let (proofs, msg_count1) = build_proofs(
        &shards[0],
        &neighbors,
        &state_roots,
        &ImportedMsgQueueLimits::new(1024 * 1024, 1024),
    )?;
    assert_eq!(msg_count1.iter().sum::<u32>(), 1024);

    // rebuild from itself
    let mut virtual_state_roots = vec![];
    for root in &proofs {
        let mproof = MerkleProof::construct_from_cell(root.clone())?;
        virtual_state_roots.push(mproof.proof.clone().virtualize(1));
    }

    let (re_proofs, msgs_count2) = build_proofs(
        &shards[0],
        &neighbors,
        &virtual_state_roots,
        &ImportedMsgQueueLimits::new(1024 * 1024, 1024),
    ...
    assert_eq!(msg_count1, msgs_count2);
    for (proof, re_proof) in proofs.iter().zip(re_proofs.iter()) {
        println!("proof:    {:#}", proof);
        println!("re_proof: {:#}\n", re_proof);
        assert_eq!(proof, re_proof);
    }

    let res = build_proofs(
        &shards[0],
        &neighbors,
        &virtual_state_roots,
        &ImportedMsgQueueLimits::new(1024 * 1024, 1024 * 2),
    );
    println!("res: {:#?}", res);
    assert!(res.is_err());
