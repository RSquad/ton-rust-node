...
use crate::{
    ...
    full_node::apply_block::calc_and_store_state,
...
use ton_block::{
    ...
    BlockSignaturesVariant, CryptoSignaturePair, Deserializable, HashmapType, McShardRecord,
    MerkleProof, Result, Serializable, TopBlockDescr, UInt256, UsageTree, ValidatorBaseInfo,
    ValidatorSet,
};
...
/// Extract CryptoSignaturePair vector from BlockSignaturesVariant
///
/// This helper converts the structured signature data back to raw pairs
/// for use by the internal proof creation functions.
fn extract_signature_pairs(variant: &BlockSignaturesVariant) -> Result<Vec<CryptoSignaturePair>> {
    let pure_sigs = variant.pure_signatures();
    let mut pairs = Vec::with_capacity(pure_sigs.count() as usize);

    pure_sigs.signatures().iterate_slices(|_key, slice| {
        let pair = CryptoSignaturePair::construct_from(&mut slice.clone())?;
        pairs.push(pair);
        Ok(true)
    })?;
...
fn can_promote_to_top_shard_descr(signatures: &BlockSignaturesVariant) -> bool {
    match signatures {
...
pub async fn accept_block(
    ...
    // Extract signature pairs from the variant
    // For Ordinary variant, we extract the pairs from pure_signatures
    // The ValidatorBaseInfo in the variant may have placeholder values (0, 0)
    // but we rebuild it from validator_set anyway in create_new_proof_internal
    let signatures = extract_signature_pairs(&signatures_variant)?;
    ...
    let block_opt = data
        .map(|data| -> Result<BlockStuff> {
            let block = BlockStuff::deserialize_block(id.clone(), Arc::new(data))?;
    ...
    let (handle, block, proof, signatures_out) = loop {
        ...
        .await
        {
            ...
            Err(e) => {
                ...
                if attempt > MAX_ATTEMPTS {
                    log::error!(
                        "({}): accept_block routine: OUT OF {MAX_ATTEMPTS} ATTEMPTS",
                        block_descr
                    );
    ...
    if id.shard().is_masterchain() {
        ...
        if can_promote_to_top_shard_descr(&signatures_out) {
            let last_mc_state = choose_mc_state(&block, &engine).await?;
            if let Some(tbd) = create_top_shard_block_description(
                &block,
                signatures_out.clone(), // Pass full BlockSignaturesVariant for proper verification
                &last_mc_state,
                prev,
                engine.deref(),
            )
            .await?
            {
                // TopBlockDescrStuff now handles both Ordinary and Simplex signature verification
                let tbd_stuff = Arc::new(TopBlockDescrStuff::new(
                    tbd,
                    block.id(),
                    is_fake,
                ...
                spawn_resend!({
                    ...
                    if let Err(e) =
                        engine.send_top_shard_block_description(tbd_stuff, cc_seqno, false).await
                    {
                        log::warn!(
                            target: "validator",
            ...
            log::debug!(
                target: "validator",
                "({}): accept_block: skipping shard top block description for non-final Simplex signatures",
                block_descr
            );
        }
    }
    ...
    if send_block_broadcast {
        ...
        // Clone signatures_out for use in the spawned task
        // Uses V1 format for Ordinary signatures (backward compatible)
        // Uses V2 format for Simplex signatures (full SignatureSet with session context)
        tokio::spawn(async move {
            log::trace!(target: "validator", "({}): accept_block: sending block broadcast", block_descr);
            if let Err(e) = engine.send_block_broadcast(&block, &proof, &signatures_out).await {
                log::warn!(
                    target: "validator",
...
pub async fn accept_block_routine(
    ...
    signatures_variant: Option<&BlockSignaturesVariant>,
    engine: &Arc<dyn EngineOperations>,
    is_fake: bool,
    is_fork: bool,
) -> Result<Option<(Arc<BlockHandle>, BlockStuff, BlockProofStuff, BlockSignaturesVariant)>> {
    ...
    let mut handle = if let Some(handle) = handle_opt {
        ...
        let handle = result
            .to_non_created()
            .ok_or_else(|| error!("INTERNAL ERROR: accept for block {} mismatch", id))?;
        #[cfg(feature = "telemetry")]
        if block_broadcast {
    ...
    calc_and_store_state(&handle, &block, &(prev[0].clone(), prev.get(1).cloned()), engine).await?;

    // Create proof using variant-aware function if Simplex variant provided
    let (proof, signatures_out) = match signatures_variant {
        Some(variant @ BlockSignaturesVariant::Simplex(_)) => {
            // Use Simplex-aware proof creation
            create_new_proof_with_variant(&block, validator_set, signatures, variant)?
        }
        _ => {
            // Use standard proof creation for Ordinary/catchain
            let (proof, sigs) = create_new_proof(&block, validator_set, signatures)?;
            (proof, BlockSignaturesVariant::Ordinary(sigs))
        }
    };

    log::debug!(target: "validator", "({}): accept_block: storing block proof", block_descr);
    handle = engine
        .store_block_proof(id, Some(handle), &proof)
        .await?
        .to_non_created()
        .ok_or_else(|| error!("INTERNAL ERROR: accept for block {} proof mismatch", id))?;

    Ok(Some((handle, block, proof, signatures_out)))
}
...
async fn choose_mc_state(
    ...
    if last_mc_state.block_id().seq_no() < mc_block_id.seq_no() {
        ...
        last_mc_state.shard_state_extra()?.prev_blocks.check_block(&mc_block_id).map_err(|_| {
            error!(
                "shardchain block {} refers to masterchain block {} \
...
pub fn create_new_proof(
    ...
    create_new_proof_internal(block_stuff, validator_set, signatures, false, None)
}

pub fn create_new_proof_link(block_stuff: &BlockStuff) -> Result<BlockProofStuff> {
    let (proof, _) =
        create_new_proof_internal(block_stuff, &ValidatorSet::default(), &Vec::new(), true, None)?;
    Ok(proof)
}

/// Create proof with Simplex signature variant support
pub fn create_new_proof_with_variant(
    block_stuff: &BlockStuff,
    validator_set: &ValidatorSet,
    signatures: &[CryptoSignaturePair],
    signatures_variant: &BlockSignaturesVariant,
) -> Result<(BlockProofStuff, BlockSignaturesVariant)> {
    create_new_proof_with_variant_internal(
        block_stuff,
        validator_set,
        signatures,
        false,
...
fn create_new_proof_internal(
    ...
    _signatures_variant: Option<&BlockSignaturesVariant>,
) -> Result<(BlockProofStuff, BlockSignatures)> {
    ...
    let merkle_proof = MerkleProof::create(block_stuff.root_cell(), |h| usage_tree.contains(h))?;

    if id.shard().is_masterchain()
        && !block.read_info()?.key_block()
        && block
            .read_extra()?
            .read_custom()?
    ...
    if force_proof_link {
        let proof =
            BlockProof { proof_for: id.clone(), root: merkle_proof.serialize()?, signatures: None };
    ...
    let checked_data = Block::build_data_for_sign(&id.root_hash, &id.file_hash);
    let weight = block_signatures
        .pure_signatures
        .check_signatures(validator_set.list(), &checked_data)
    ...
    if weight * 3 <= total_weight * 2 {
        fail!(
            "Block {}: too small signatures weight (weight: {}, total: {})",
            id,
            weight,
            total_weight
        );
    ...
    let proof = BlockProof::with_ordinary_signatures(
        id.clone(),
        merkle_proof.serialize()?,
        if !is_link { Some(block_signatures.clone()) } else { None },
    );

    Ok((BlockProofStuff::new(proof, is_link)?, block_signatures))
}

/// Create proof with proper signature variant handling
///
/// For Simplex variant, uses the consensus `DataToSign(session_id, vote)` format (with
/// `CandidateHashData`) instead of the standard `Block::build_data_for_sign()`.
fn create_new_proof_with_variant_internal(
    block_stuff: &BlockStuff,
    validator_set: &ValidatorSet,
    signatures: &[CryptoSignaturePair],
    force_proof_link: bool,
    ...
    let id = block_stuff.id();
    let block_descr = fmt_block_id_short(id);
    log::debug!(target: "validator", "({}): accept_block: create_new_proof_with_variant", block_descr);
    ...
    if force_proof_link {
        let proof =
            BlockProof { proof_for: id.clone(), root: merkle_proof.serialize()?, signatures: None };
        return Ok((
            BlockProofStuff::new(proof, true)?,
            BlockSignaturesVariant::Ordinary(BlockSignatures::default()),
        ));
    }

    // build BlockSignaturesPure struct
    let total_weight = validator_set.total_weight();
    let mut block_signatures_pure = BlockSignaturesPure::with_weight(total_weight);
    for sign in signatures {
        block_signatures_pure.add_sigpair(sign.clone())
    ...
    // Check signatures using the appropriate signed data format
    let checked_data = match signatures_variant {
        Some(BlockSignaturesVariant::Simplex(simplex)) => {
            // For Simplex, use the stored context to build the signed data.
            // Also validate that the embedded candidate refers to this block id.
            simplex_to_sign_checked(simplex, id)
                .map_err(|e| error!("Failed to build Simplex signed data for block {id}: {e}"))?
        }
        _ => {
            // For Ordinary/catchain, use the standard block-based format
            Block::build_data_for_sign(&id.root_hash, &id.file_hash).to_vec()
        }
    };

    let weight = block_signatures_pure
        .check_signatures(validator_set.list(), &checked_data)
        .map_err(|e| error!("Error while check signatures for block {}: {}", id, e))?;

    block_signatures_pure.set_weight(weight);
    ...
    // Build the output signature variant, preserving the type from input
    let output_variant = match signatures_variant {
        Some(BlockSignaturesVariant::Simplex(simplex)) => {
            // Preserve Simplex variant with updated validator_info and weight
            let mut updated = simplex.clone();
            updated.set_validator_info(validator_info);
            updated.pure_signatures_mut().set_weight(weight);
            BlockSignaturesVariant::Simplex(updated)
        }
        _ => {
            // Create Ordinary variant (default for catchain)
    ...
    let proof = BlockProof {
        ...
        signatures: if !is_link { Some(output_variant.clone()) } else { None },
    };

    Ok((BlockProofStuff::new(proof, is_link)?, output_variant))
}
...
pub fn visit_block_for_proof(block: &Block, id: &BlockIdExt) -> Result<()> {
    ...
    if !id.shard().is_masterchain() {
        extra
            .read_wc_custom()?
            .ref_shard_blocks
            .iterate(|shards| shards.iterate(|_prefix, _block_id| Ok(true)))?;
    }
    ...
    if info.key_block() {
        let mc_extra = mc_extra.as_ref().ok_or_else(|| {
            error!("can not extract masterchain block extra from key block {}", id)
        })?;

        let config = mc_extra.config().ok_or_else(|| {
            error!("can not extract config params from masterchain block extra of block {}", id)
        })?;
...
pub async fn create_top_shard_block_description(
    ...
    if let Some((oldest_ancestor_seqno, ancestors)) = find_known_ancestors(block, mc_state)? {
        let proof_links =
            build_proof_chain(block, prev, oldest_ancestor_seqno, ancestors, engine, mc_state)
                .await?;
        ...
        for proof in proof_links {
            ...
            //     proof.proof_root()
            // );
            tbd.append_proof(proof.merkle_proof_root_cell().clone());
        }
...
fn find_known_ancestors(
    ...
    let master_ref = block
        ...
        .read_info()?
        .read_master_ref()?
        .ok_or_else(|| error!("Block {} doesn't have `master_ref`", block.id()))?
        .master;
    ...
    match mc_state_extra.shards().find_shard(shard) {
        Ok(None) => {
            ...
            if let (Some(ancestor1), Some(ancestor2)) = (ancestor1, ancestor2) {
                ...
                oldest_ancestor_seqno =
                    max(ancestor1.block_id().seq_no(), ancestor2.block_id().seq_no());
...
async fn build_proof_chain(
    ...
    let handle = engine
        .load_block_handle(block.id())?
        .ok_or_else(|| error!("Cannot load handle for block {}", block.id()))?;
    ...
    loop {
        ...
        let prev_handle = engine
            .load_block_handle(&link_prev[0])?
            .ok_or_else(|| error!("Cannot load handle for prev block {}", link_prev[0]))?;
...
fn validate_proof_link(
    ...
    let (_, prev_stuff) = construct_and_check_prev_stuff(&virt_block_root, proof_link.id(), false)
        .map_err(|e| {
            error!("error in block header in proof link for {}: {}", proof_link.id(), e)
        })?;
    ...
    if prev_stuff.mc_block_id.seq_no() > mc_block_id.seq_no() {
        ...
        mc_state.shard_state_extra()?.prev_blocks.check_block(&prev_stuff.mc_block_id).map_err(
            |err| {
                error!(
