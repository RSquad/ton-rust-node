 ...
 */
#[cfg(feature = "xp25")]
use crate::types::top_block_descr::TopBlockDescrGroup;
use crate::{
    block::BlockStuff,
    ...
    internal_db::{
        restore::set_graceful_termination, BlockResult, InternalDb, InternalDbConfig,
        CURRENT_DB_VERSION,
    },
    shard_state::ShardStateStuff,
    test_helper::{are_shard_states_equal, init_test_log, WaitForHandle},
...
use storage::{
    block_handle_db::{BlockHandle, Callback},
    shardstate_db_async::SsNotificationCallback,
    types::{BlockMeta, PersistentStatePartId},
};
use ton_block::{
    error, fail, sha256_digest_slices, BlockIdExt, BlockSignatures, Result, Serializable,
    ShardIdent, ShardStateUnsplit, TopBlockDescr, UInt256,
};

const DB_PATH: &str = "../target/test";

async fn create_db(test_name: &str, monitor_min_split: u8) -> Result<InternalDb> {
    let attempts = 5;
    for _ in 0..attempts {
...
async fn clean_up(may_fail: bool, test_name: &str) {
    loop {
        if std::fs::remove_dir_all(format!("{}/{}", DB_PATH, test_name)).is_ok() || may_fail {
            break;
        }
...
fn prepare_block_only() -> Result<Data<()>> {
    let ret = Data { block: prepare_block()?, extra: () };
    Ok(ret)
}
...
async fn test_store<X>(
    ...
    for i in 0..2 {
        let ctx = Context { db: create_db(name, 0).await?, data: &data, wait: wait.clone() };
        ...
        let handle = if i == 0 {
            ...
            ctx.db
                .load_block_handle(ctx.data.block.id())?
                .ok_or_else(|| error!("Cannot load handle for block {}", ctx.data.block.id()))?
        };
        let job = Job { ctx, handle };
        if i == 0 {
            store(&job)?;
        }
        check(&job).await?;
...
async fn test_store_block_proof() {
    ...
    test_store_with_clean(
        "test_store_block_proof",
        prepare_block_proof,
        |c| {
            Box::pin(c.db.store_block_proof(
                c.data.extra.id(),
                None,
                &c.data.extra,
                Some(c.wait.clone()),
...
async fn test_store_ss_persistent_impl() -> Result<()> {
    let db = create_db("test_store_ss_persistent", 0).await?;
    ...
    let block_handle = db
        .store_block_data(&block, Some(wait.clone()))
        .await?
        .to_updated()
        .ok_or_else(|| error!("Bad result in store block {} data", block.id()))?;
    ...
    let size = db
        .load_shard_state_persistent_size(&PersistentStatePartId::WholeState(ss.block_id().clone()))
        .await?;
    let data = db
        .load_shard_state_persistent_slice(
            &PersistentStatePartId::WholeState(ss.block_id().clone()),
            0,
            size,
        )
...
async fn test_top_shard_blocks_db() {
    clean_up(true, "test_top_shard_blocks_db").await;
...
async fn test_top_shard_blocks_db_impl() -> Result<()> {
    {
        let db = create_db("test_top_shard_blocks_db", 0).await?;
        ...
        for seqno in 0..=100 {
            for shard in 0..=0xf {
                for cc_seqno in 44..=45 {
                    ...
                    let tbds_id = TopBlockDescrId { id: id.shard_id.clone(), cc_seqno };
                    #[cfg(feature = "xp25")]
                    let tbds_id = TopBlockDescrId {
                        group: TopBlockDescrGroup { shard_ident: id.shard_id.clone(), cc_seqno },
                        seqno: id.seq_no(),
                    };

                    let tbd = TopBlockDescr::with_id_and_ordinary_signatures(
                        id.clone(),
        ...
        for _seqno in 0..=100 {
            for shard in 0..=0xf {
                ...
                #[cfg(not(feature = "xp25"))]
                let tbds_id = TopBlockDescrId { id: shard_ident, cc_seqno: 44 };
                #[cfg(feature = "xp25")]
                let tbds_id = TopBlockDescrId {
                    group: TopBlockDescrGroup { shard_ident, cc_seqno: 44 },
                    seqno: _seqno,
                };
    ...
    tokio::time::sleep(Duration::from_millis(100)).await;
    {
        ...
        for _seqno in 0..=100 {
            for shard in 0..=0xf {
                ...
                #[cfg(not(feature = "xp25"))]
                let tbds_id = TopBlockDescrId { id: shard_ident, cc_seqno: 45 };
                #[cfg(feature = "xp25")]
                let tbds_id = TopBlockDescrId {
                    group: TopBlockDescrGroup { shard_ident, cc_seqno: 45 },
                    seqno: _seqno,
                };
...
async fn test_full_node_state_impl() -> Result<()> {
    ...
    {
        let db = create_db("test_full_node_state", 0).await?;
        ...
        tokio::time::sleep(Duration::from_millis(100)).await; // for update db
...
fn shard_prefix(shard_prefix: u64) -> ShardIdent {
    let shard_prefix = shard_prefix << (64 - SHARD_PREFIX_LEN);
    ShardIdent::with_prefix_len(SHARD_PREFIX_LEN, 0, shard_prefix).unwrap()
}

async fn check_block(db: &InternalDb, id: &BlockIdExt) {
    let handle = db.load_block_handle(id).unwrap().unwrap();
    let mut is_link = false;
    assert!(handle.is_archived());
    assert!(handle.is_applied());
    assert!(handle.has_data());
    assert!(handle.has_proof_or_link(&mut is_link));
    db.load_block_data_raw(&handle).await.unwrap();
    db.load_block_proof_raw(&handle, is_link).await.unwrap();
}
...
async fn store_block(
    ...
    let handle = db
        .store_block_data(&BlockStuff::fake_block(id.clone(), mc.cloned(), false)?, None)
        .await?
        .to_any();
    let handle = db
        .store_block_proof(id, Some(handle), &BlockProofStuff::fake(id)?, None)
        .await?
        .to_non_created()
        .ok_or_else(|| error!("Bad result in store block {} proof", id))?;
    if let Some(mc) = mc {
        db.assign_mc_ref_seq_no(&handle, mc.seq_no, None)?;
    }
    Ok(handle)
}

async fn test_archives(multithread: bool) {
    const SHARDS: u64 = 1 << SHARD_PREFIX_LEN;
    let db_name: &str =
        if multithread { "test_archives_multithread" } else { "test_archives_singlethread" };
    ...
    clean_up(true, db_name).await;
    let db = Arc::new(create_db(db_name, 2).await.unwrap());

    // Initial blocks created separately because archive slice creation is
    // not multithread-safe (TODO)
    // master
    ...
    for mc_seq_no in 1..MC_BLOCKS {
        ...
        for _ in 0..THREADS {
            ...
            let mc_id_clone = mc_id.clone();
            let op = async move || {
                let handle = store_block(db_clone.as_ref(), &mc_id_clone, None).await.unwrap();
                db_clone.archive_block(&mc_id_clone, None).await.unwrap();
                db_clone.store_block_applied(&handle, None).unwrap();
            };
            if multithread {
    ...
    println!("Written in {}ms", now.elapsed().as_millis());
    stop_db(&db).await;
    drop(db);
    ...
    let mut mc_count = 0;
    let mut sh_count = 0;
    for mc_seq_no in 0..MC_BLOCKS {
        check_block(&db, &gen_block_id_ext(ShardIdent::masterchain(), mc_seq_no)).await;
        mc_count += 1;
        for shard_byte in 0..SHARDS {
            check_block(&db, &gen_block_id_ext(shard_prefix(shard_byte), mc_seq_no)).await;
            sh_count += 1;
        }
    }
    println!("Read {mc_count} master blocks and {sh_count} shard blocks");

    stop_db(&db).await;
    drop(db);
    clean_up(false, db_name).await;
}

#[tokio::test(flavor = "multi_thread")]
async fn test_archives_multithread() {
    test_archives(true).await;
}

#[tokio::test(flavor = "multi_thread")]
async fn test_archives_singlethread() {
    test_archives(false).await;
}

#[tokio::test(flavor = "multi_thread")]
async fn test_store_ss_lite() {
    init_test_log();
    clean_up(true, "test_store_ss_lite").await;
...
async fn test_store_ss_lite_impl(path: &str) -> Result<()> {
    ...
    let cb = SsNotificationCallback::new();
    let (ss1, _) = db
        .store_shard_state_dynamic(&block_handle, &ss1, Some(wait.clone()), Some(cb.clone()), false)
        .await?;
    cb.wait().await;
...
async fn test_shard_state_persistent_gc_impl() -> Result<()> {
    let db = create_db("test_shard_state_persistent_gc", 0).await?;
    ...
    for (id, time) in &key_blocks[1..] {
        if engine.is_persistent_state(*time, prev_time, crate::boot::PSS_PERIOD_BITS) {
            let id = BlockIdExt { root_hash: id.parse()?, ..Default::default() };
            let mut meta = BlockMeta::default();
            meta.gen_utime = *time;
            let handle = db.block_handle_storage.create_handle(id.clone(), meta, None)?.unwrap();
            db.store_shard_state_persistent_raw(&handle, &vec![*time as u8; 1024], None).await?;
        }
        prev_time = *time;
    }
