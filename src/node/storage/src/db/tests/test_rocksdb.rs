...
use crate::{
    db::{
        rocksdb::{destroy_rocks_db, AccessType, RocksDb, RocksDbTable},
        tests::utils::{expect_error, expect_key_not_found_error},
    },
    error::StorageError,
};
use rocksdb::{BlockBasedOptions, Cache, DBWithThreadMode, MultiThreaded, Options, WriteBatch, DB};
...
async fn test_foreach() -> Result<()> {
    ...
    let mut index = 0;
    let result = tb.for_each(&mut |key, value| {
    ...
    let snapshot = tb.snapshot()?;
    index = 0;
    let result = snapshot.for_each(&mut |key, value| {
        let (key0, value0) = vec.get(index).unwrap();
        index += 1;
        assert_eq!(key, *key0);
        assert_eq!(value, &[*value0]);
...
async fn test_db_open_and_create_column_family() {
    const DB_NAME: &str = "family_db_open_create";
...
async fn test_double_create() {
    const DB_NAME: &str = "family_db_double_create";
