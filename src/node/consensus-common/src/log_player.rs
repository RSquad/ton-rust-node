 ...
 */
use crate::{
    utils, BlockPayloadPtr, ConsensusCommonFactory, ConsensusNode, ConsensusOverlay,
    ConsensusOverlayListenerPtr, ConsensusOverlayLogReplayListenerPtr, ConsensusOverlayManager,
    ConsensusOverlayManagerPtr, ConsensusOverlayPtr, ConsensusReplayListenerPtr, LogPlayer,
    LogPlayerPtr, LogReplayOptions, OverlayTransportType, PrivateKey, PublicKeyHash,
    QueryResponseCallback, SessionId, ValidatorWeight,
};
...
impl fmt::Debug for LogReplayOptions {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LogReplayOptions").field("log_file_name", &self.log_file_name).finish()
    }
}
...
#[derive(Clone)]
struct SessionDesc {
    session_id: SessionId,           //consensus session ID
    node_ids: Vec<ConsensusNode>,    //list of validators
...
struct LogHeader {
    session_descs: Vec<SessionDesc>, //consensus sessions in a log
...
impl LogHeader {
    ...
    fn add_session_id(&mut self, session_id: SessionId, timestamp: SystemTime) {
        ...
        self.session_index.insert(session_id, self.session_descs.len() - 1);
    }
    ...
    fn add_node(
        ...
        for node in &session_desc.node_ids {
            if node.adnl_id == consensus_node.adnl_id {
                return;
            }
        }
        ...
        log::trace!(
            ...
            consensus_node.adnl_id,
            consensus_node.public_key.id(),
            weight,
            session_id
        );

        session_desc.node_ids.push(consensus_node);
...
impl Format {
    ...
    where
        ...
        F: 'static,
    {
        Ok(Self { regexp: regex::Regex::new(regexp)?, parser: Box::new(parser_fn) })
    }
}
...
struct LogPlayerOverlayManager {
    ...
    initial_timestamp: SystemTime,               //initial timestamp for replaying
    replay_listener: ConsensusReplayListenerPtr, //replay listener
}

impl ConsensusOverlayManager for LogPlayerOverlayManager {
    /// Create new overlay
    fn start_overlay(
        &self,
        ...
        let _processing_thread =
            std::thread::Builder::new().name(MAIN_LOOP_NAME.to_string()).spawn(move || {
        ...
        let result: ConsensusOverlayPtr = overlay;
...
impl LogPlayerImpl {
    ...
    */
    ...
    fn main_loop(
        ...
        log::debug!("LogReplay main loop is started with replay_options={:?}", log_replay_options);
        ...
        log::debug!("LogReplay main loop is finished in {:?}", start_time.elapsed());
    ...
    */
    ...
    fn parse(path: &str, formats: &[Format], stop_flag: &Arc<AtomicBool>) -> Result<()> {
        ...
        for (index, line) in reader.lines().enumerate() {
            ...
            if let Err(err) = line {
                log::warn!("LogReplay: parsing error (line #{}): {:?}", line_number, err);
                continue;
            }
    ...
    fn parse_header(
        ...
        let add_session_id = move |session_id: SessionId, timestamp: SystemTime| {
            log_header_clone.borrow_mut().add_session_id(session_id, timestamp);
        };
        let log_header_clone = log_header.clone();
        let add_local_id = move |session_id: SessionId, local_id: PublicKeyHash| {
            log_header_clone.borrow_mut().add_local_id(session_id, local_id);
        };
        let log_header_clone = log_header.clone();
        let add_private_key = move |public_key_hash: PublicKeyHash, private_key: PrivateKey| {
            log_header_clone.borrow_mut().add_private_key(public_key_hash, private_key);
        };
        let log_header_clone = log_header.clone();
        let add_node = move |session_id: SessionId, consensus_node: ConsensusNode, weight| {
            log_header_clone.borrow_mut().add_node(session_id, consensus_node, weight);
        };
        ...
        let formats = [
            Format::new(
                ...
                move |captures: &regex::Captures| {
                    let session_id =
                        utils::parse_hex_as_session_id(captures.get(1).unwrap().as_str());
                    let local_id =
                        utils::parse_hex_as_public_key_hash(captures.get(2).unwrap().as_str());
            ...
            Format::new(
                ...
                move |captures: &regex::Captures| {
                    let session_id =
                        utils::parse_hex_as_session_id(captures.get(1).unwrap().as_str());
                    let weight = captures.get(2).unwrap().as_str().parse::<ValidatorWeight>()?;
                    let public_key =
                        utils::parse_hex_as_public_key(captures.get(3).unwrap().as_str());
                    let adnl_id =
                        utils::parse_hex_as_public_key_hash(captures.get(4).unwrap().as_str());
                    let consensus_node = ConsensusNode { public_key, adnl_id };

                    add_node_clone(session_id, consensus_node, weight);
            ...
            Format::new(
                ...
                move |captures: &regex::Captures| {
                    ...
                    let consensus_node = ConsensusNode { public_key, adnl_id };
                    let weight = captures.get(5).unwrap().as_str().parse::<ValidatorWeight>()?;

                    add_node(session_id, consensus_node, weight);
    ...
    */
    ...
    pub fn create_log_player(log_replay_options: &LogReplayOptions) -> Result<LogPlayerPtr> {
        log::debug!("LogPlayer: created with replay_options={:?}", log_replay_options);
        ...
        if log_header.session_descs.is_empty() {
            fail!("No sessions have been found in the log '{}'", &log_replay_options.log_file_name);
        }
        ...
        let preferred_session_id =
            if let Some(preferred_session_id) = &log_replay_options.session_id {
                ...
                log_header.session_descs[log_header.session_descs.len() - 1].session_id.clone()
            };
        let session_desc = log_header.get_session(&preferred_session_id);

        if session_desc.is_none() {
            fail!(
                "Session ID {:x} has not been found in the log '{}'",
...
struct OverlayImpl {
    ...
    listener: ConsensusOverlayListenerPtr, //overlay listener
}

impl ConsensusOverlay for OverlayImpl {
    ...
    fn send_message(
        ...
        log::debug!("LogReplay: send message {} -> {}: {:?}", sender_id, receiver_id, message);
    }
    ...
    fn send_query_via_rldp(
        ...
        log::debug!("LogReplay: send query '{}' via RLDP -> {}: {:?}", name, dst, query);
    }
...
impl OverlayImpl {
    ...
    fn new(
        ...
        Self { listener, should_stop_flag, is_stopped_flag }
    }
}
