...
impl Profiler {
    ...
    fn new() -> Self {
        ...
        let mut profiler = Profiler { spans: Vec::with_capacity(SPANS_POOL_SIZE), current_span: 0 };
    ...
    fn accumulate_span(
        ...
        assert!(std::ptr::eq(src_span.code_line, dst_span.code_line));
        ...
        while src_child_span_index != -1 {
            ...
            self.accumulate_span(src_profiler, src_child_span_index as usize, dst_child_span_index);
    ...
    pub fn dump(&self) -> String {
        ...
        let result = format!("{}\n{}", result, self.dump_spans_self_time(&self_time_spans));
    ...
    fn dump_span(
        ...
        let indent = String::from_utf8(vec![b' '; (depth + 1) * 2]).unwrap().to_string();
        ...
        let total_duration =
            if parent_span_index == 0 { total_children_duration } else { parent_span.duration };
...
impl ProfileGuard {
    pub fn new(code_line: &'static ProfileCodeLine) -> Self {
        let body = Self { start_time: std::time::Instant::now() };
...
impl ExecutionTimeGuard {
    pub fn new(
        ...
        Self { start_time: std::time::Instant::now(), code_line, max_duration }
    }
}
...
macro_rules! instrument {
    () => {
        ...
        let _instrument_guard = $crate::profiling::ProfileGuard::new(&INSTRUMENT_CODE_LINE);
    };
}
