 ...
 */
use catchain::{
    utils::*, BlockPayloadPtr, BlockPtr, CatchainFactory, CatchainListener, CatchainNode, Options,
    PublicKeyHash, QueryResponseCallback, SessionId,
};
...
impl CatchainListener for DummyCatchainListener {
    fn preprocess_block(&self, block: BlockPtr) {
        log::info!("DummyCatchainListener::preprocess_block: {:?}", block);
    }

    fn process_blocks(&self, blocks: Vec<BlockPtr>) {
        log::info!("DummyCatchainListener::process_blocks: {:?}", blocks);
    }

    fn finished_processing(&self) {
        log::info!("DummyCatchainListener::finished_processing");
    }

    fn started(&self) {
        log::info!("DummyCatchainListener::started");
    }
...
";
    ...
    for src in sources.lines() {
        if let Some(hex) = src.split(": ").nth(1) {
            ...
            source_ids.push(CatchainNode { public_key, adnl_id });
        }
    }
    let local_key = source_ids[0].public_key.clone();

    let catchain_listener =
        Arc::new(DummyCatchainListener) as Arc<dyn CatchainListener + Send + Sync>;
