...
fn compute_hash_impl(
    ...
    opts.block_hash_covers_data = false;
    use catchain::ton::Block;
    let mut block = Block::default();
    block.incarnation = incarn.clone();
    block.height = height;
    let block_id = get_block_id(&incarn, &pk_hash, &block, &pld_vec, &opts);
    println!(
...
fn compute_hash(incarnation: &str, source_public_key_hash: &str, block: &str) -> BlockHash {
    ...
    compute_hash_impl(incarnation, source_public_key_hash, &bytes_to_string(&pld), blk.block.height)
}
...
fn test_payload_hash() {
    for (pl, hc) in &[
        ...
        assert_eq!(get_hash(&parse_hex(pl)), parse_hex_as_int256(hc));
    }
}
...
fn test_block_id_hash() {
    for (inc, pk, pld, height, hash) in &[
        ...
        assert_eq!(compute_hash_impl(inc, pk, pld, *height), parse_hex_as_int256(hash));
    }
}

#[test]
fn test_block_hash() {
    let (inc, pk, blk, hash) = ("2a3c82246129cf61307839b7f459cceb9967e0fba9959391fa60beaabbec7931", 
