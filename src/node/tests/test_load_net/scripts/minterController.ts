...
import { compile, NetworkProvider, SenderWithSendResult, UIProvider } from '@ton/blueprint';
...
import { promptBool, promptAmount, promptAddress, displayContentCell, waitForTransaction, promptToncoin, promptUrl, getAccountLt } from '../wrappers/ui-utils';
...
const infoAction = async (_provider: NetworkProvider, ui: UIProvider) => {
...
const topUpAction = async (ui: UIProvider) => {
    let sender = await (await AppGlobals.S()).getFaucetWallet().getSender();
    ...
    await jettonMinterContract.sendTopUp(sender, topUpAmount);
}

const updateMetadataAction = async (ui: UIProvider) => {
    ...
    await jettonMinterContract.sendChangeContent(sender, {
...
const changeAdminAction = async (provider: NetworkProvider, ui: UIProvider) => {
    ...
    const lt = (await getAccountLt(provider, jettonMinterContract.address))!;
    let sender = await (await AppGlobals.S()).getFaucetWallet().getSender();

    await jettonMinterContract.sendChangeAdmin(sender, newAdmin);
...
const dropAdminAction = async (ui: UIProvider) => {
    let curAdmin = await jettonMinterContract.getAdminAddress();
    let retry: boolean;
    ...
    if (sure) {
        let sender = await (await AppGlobals.S()).getFaucetWallet().getSender();
        await jettonMinterContract.sendDropAdmin(sender);
    }
    else {
...
const claimAdminAction = async (provider: NetworkProvider, ui: UIProvider) => {
    ...
    let sender = await (await AppGlobals.S()).getFaucetWallet().getSender();
    await jettonMinterContract.sendClaimAdmin(sender);
...
const upgradeAction = async (provider: NetworkProvider, ui: UIProvider) => {
    ...
    const contractState = await api.getContractState(/*await getLastBlock(provider),*/ jettonMinterContract.address);

    //if (contractState.account.state.type !== 'active')
    if (contractState.state !== 'active')
        throw (Error("Upgrade is only possible for active contract"));

    if (contractState.code === null)
        throw (Error(`Something is wrong!\nActive contract has to have code`));

    const dataBefore = contractState.data ? Cell.fromBoc(contractState.data!)[0] : beginCell().endCell();
    if (upgradeCode || upgradeData) {
        const newCode = upgradeCode ? minterCode : Cell.fromBoc(contractState.code!)[0];
        const newData = upgradeData ? await updateData(dataBefore, ui) : dataBefore;
        let sender = await (await AppGlobals.S()).getFaucetWallet().getSender();
        await jettonMinterContract.sendUpgrade(sender, newCode, newData, toNano('0.05'));
        const gotTrans = await waitForTransaction(provider,
            jettonMinterContract.address,
...
export async function run(provider: NetworkProvider) {
    ...
    let done = false;
    let retry: boolean;
    let minterAddress = Address.parse(process.env.JETTON_MINTER!);
    ...
    do {
        ...
        switch (action) {
            ...
            case 'Change metadata':
                await updateMetadataAction(ui);
                break;
            case 'Claim admin':
                await claimAdminAction(provider, ui);
                break;
            case 'Drop admin':
                await dropAdminAction(ui);
                break;
            case 'Upgrade':
