...
//   return (await (provider.api() as TonClient).getLastBlock()).last.seqno;
//}

export const getAccountLt = async (provider: NetworkProvider, address: Address) => {
    //const res = await (provider.api() as TonClient).getAccountLite(await getLastBlock(provider), address);
    //if (res.account.last == null)
    //    throw (Error("Contract is not active"));
    //return res.account.last.lt;

    const contractState = await (provider.api() as TonClient).getContractState(/*await getLastBlock(provider),*/ address);
    return contractState.lastTransaction?.lt;
}

export const waitForTransaction = async (provider: NetworkProvider, address: Address, curLT: string | null, maxRetry: number, interval: number = 1000) => {
    let done = false;
    let count = 0;
    ...
    do {
        ...
        //const curState = await (provider.api() as TonClient).getAccountLite(lastBlock, address);
        const lt = await getAccountLt(provider, address);
        done = lt !== curLT;
