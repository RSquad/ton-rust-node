use crate::{
    rng::rand,
    systematic_constants::{MAX_SOURCE_SYMBOLS_PER_BLOCK, SYSTEMATIC_INDICES_AND_PARAMETERS},
    util::int_div_ceil,
};
#[cfg(not(feature = "std"))]
use alloc::vec::Vec;
...
impl PayloadId {
    pub fn new(source_block_number: u8, encoding_symbol_id: u32) -> PayloadId {
        ...
        PayloadId { source_block_number, encoding_symbol_id }
    }
...
pub fn intermediate_tuple(
    ...
    let d1 = if d < 4 { 2 + rand(internal_symbol_id, 3u32, 2) } else { 2 };
