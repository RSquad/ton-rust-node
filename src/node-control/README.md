...
- `pools` — JSON array of **exactly two** elements: `pools[0]` and `pools[1]`. Each element is either `null` (slot unused) or an object:
  ...
  - `deploy_layout` — optional string per slot: `embedded_code` (default — entire pool bytecode in `StateInit.code`) or `activate_upgrade` (minimal bootstrap in `code`, full pool installed via `SETCODE` on first execution — often described as “upgrade on activation”; delivery is still normally an internal deploy message with `state_init`, not necessarily an external message).
