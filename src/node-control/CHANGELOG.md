...
- **In-pod vault migration via `nodectl key migrate`** — new CLI command that copies all secrets from `FROM_VAULT_URL` to `VAULT_URL` without leaving the Pod and without any extra binary. Supports `--dry-run`, `--on-conflict <fail|skip|overwrite>`, `--list-mode <only-needed|all>`, and `--continue-on-error`. For file → HashiCorp migration of non-extractable Ed25519 keys (which file storage keeps at rest under master-key encryption, but whose policy flag forbids export), pass `--allow-nonexportable` to opt into the raw-bytes path; the destination key is still created with `exportable=false` in Vault, so the security property is preserved. The flag has no effect for any other source/destination combination. See `helm/nodectl/docs/copy-file-to-hashicorp.md`.
