...
fn test_cell_draft() {
    // Leaf cell (no refs)
    let leaf = Cell::with_data_and_refs(&[0, 0], false, &[], None, None).unwrap();
    let draft_leaf = Cell::new_boc_draft(&[0, 0], None, &[]).unwrap();
    let leaf2 = Cell::from_boc_draft(draft_leaf, &[], None, None).unwrap();
    assert_eq!(*leaf.repr_hash(), *leaf2.repr_hash());
    assert_eq!(leaf.repr_depth(), leaf2.repr_depth());

    // Cell with data
    let raw = {
        let mut b = BuilderData::new();
        b.append_u32(0xDEAD_BEEF).unwrap();
        b.append_u32(0xCAFE_BABE).unwrap();
        let cell = b.into_cell().unwrap();
        cell.raw_data().unwrap().to_vec()
    };
    let c1 = Cell::with_data_and_refs(&raw, false, &[], None, None).unwrap();
    let draft = Cell::new_boc_draft(&raw, None, &[]).unwrap();
    let c2 = Cell::from_boc_draft(draft, &[], None, None).unwrap();
    assert_eq!(*c1.repr_hash(), *c2.repr_hash());

    // Cell with references
    let child1 = Cell::with_data_and_refs(&[0, 0], false, &[], None, None).unwrap();
    let child2 = {
        let mut b = BuilderData::new();
        b.append_u32(0x12345678).unwrap();
        b.into_cell().unwrap()
    };
    let raw_parent = {
        let mut b = BuilderData::new();
        b.append_u64(0xAAAA_BBBB_CCCC_DDDD).unwrap();
        b.checked_append_reference(child1.clone()).unwrap();
        b.checked_append_reference(child2.clone()).unwrap();
        let parent = b.into_cell().unwrap();
        parent.raw_data().unwrap().to_vec()
    };
    let parent1 =
        Cell::with_data_and_refs(&raw_parent, false, &[child1.clone(), child2.clone()], None, None)
            .unwrap();
    let draft = Cell::new_boc_draft(&raw_parent, None, &[1, 2]).unwrap();
    let parent2 =
        Cell::from_boc_draft(draft, &[child1.clone(), child2.clone()], None, None).unwrap();
    assert_eq!(*parent1.repr_hash(), *parent2.repr_hash());
    assert_eq!(parent1.repr_depth(), parent2.repr_depth());
    assert_eq!(parent1.references_count(), parent2.references_count());
    assert_eq!(
        *parent1.reference(0).unwrap().repr_hash(),
        *parent2.reference(0).unwrap().repr_hash()
    );
    assert_eq!(
        *parent1.reference(1).unwrap().repr_hash(),
        *parent2.reference(1).unwrap().repr_hash()
    );

    // Draft with wrong refs count should fail
    assert!(Cell::new_boc_draft(&raw_parent, None, &[1]).is_err());
    let draft = Cell::new_boc_draft(&raw_parent, None, &[1, 2]).unwrap();
    Cell::from_boc_draft(draft, &[child1.clone()], None, None).expect_err("wrong refs count");

    // Draft drop without finalize (should not leak/crash)
    let _draft = Cell::new_boc_draft(&raw_parent, None, &[1, 2]).unwrap();
}
